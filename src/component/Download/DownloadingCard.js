...
export default function DownloadingCard(props) {
    ...
    const deleteFile = (index) => {
        ...
        API.put("/aria2/select/" + task.info.gid, {
            ...
            .then(() => {
                ...
                ToggleSnackbar("top", "right", t('File has been deleted'), "success");
            })
            .catch((error) => {
    ...
    const getDownloadName = useCallback(() => {
        ...
        return task.name === "." ? t('[unknown]') : task.name;
    }, [task]);
        ...
        API.delete("/aria2/task/" + task.info.gid)
            .then(() => {
                ToggleSnackbar(
                    "top",
                    "right",
                    t('Task has been cancelled, status will be updated later'),
                    "success"
                );
            })
            .catch((error) => {
        ...
        API.put("/aria2/select/" + task.info.gid, {
            ...
            .then(() => {
                ToggleSnackbar(
                    "top",
                    "right",
                    t('The operation is successful, the status will be updated later'),
                    "success"
                );
